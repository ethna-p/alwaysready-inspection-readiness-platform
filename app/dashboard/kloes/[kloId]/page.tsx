...
export default async function KloeDetailPage({ params }: Props) {
  ...
  const { data: klo } = await supabase
    .from('klo_items')
    .select('*, key_questions(name, description)')
    .eq('id', kloId)
    .single()
  ...
  const kqName = (klo as unknown as { key_questions: { name: string; description: string | null } | null })
    ?.key_questions?.name ?? '—'
  const kqDescription = (klo as unknown as { key_questions: { name: string; description: string | null } | null })
    ?.key_questions?.description ?? null
  ...
  return (
    <div className="max-w-4xl">
      ...
      <div className="space-y-6">
        ...
        {/* CQC rating characteristics — top shortcut */}
