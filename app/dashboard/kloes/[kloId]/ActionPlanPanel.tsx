...
/**
 * ActionPlanPanel — create, assign, and sign off action items for a KLOE.
 *
 * Open items appear first (sorted by due date, then priority).
 * Completed items are retained below in a collapsible history section.
 *
 * Permissions:
 *   admin  → full access: create, edit, assign, sign off, delete
...
function AddActionForm({
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-3 bg-canvas rounded-xl border border-line p-4">
      ...
      <div>
        <label className="block text-xs font-medium text-ink-dim mb-1">Title <span className="text-red-500">*</span></label>
        <input name="title" required placeholder="e.g. Update medication policy" className={inputClass} />
      </div>
...
function SignOffForm({
  ...
  async function handleSubmit(e: React.FormEvent<HTMLFormElement>) {
    ...
    const fd = new FormData(e.currentTarget)
    fd.set('id', item.id)
    fd.set('klo_item_id', item.klo_item_id)
    startTransition(async () => {
      const result = await signOffActionItem(fd)
      if (!result.success) { setError(result.error); return }
      onDone()
    })
  }
...
function ActionItemRow({
  ...
  async function handleDelete() {
    ...
    fd.set('klo_item_id', item.klo_item_id)
    startTransition(async () => {
      const result = await deleteActionItem(fd)
  ...
  return (
    <div className="py-3 px-4 space-y-1.5">
      ...
      <div className="flex flex-wrap items-center gap-x-3 gap-y-1 text-xs text-ink-dim">
        ...
        {!assigneeName && <span>Unassigned</span>}
      </div>
  ...
  return (
    <div className="space-y-4">
      ...
      {openItems.length === 0 && !showAddForm && (
        <p className="text-sm text-ink-dim">No open action items for this KLOE.</p>
      )}
