...
export default function HrHolidaySection({ userId, allowances, holidayUnit }: Props) {
  ...
  const remaining = (parseFloat(total) || 0) + (parseFloat(carryOver) || 0) - (parseFloat(taken) || 0)
  ...
  return (
    <section className="mb-8">
      <h2 className="text-base font-semibold text-[#014D4E] mb-4">
        Holiday Allowance <span className="text-xs font-normal text-gray-500">({holidayUnit})</span>
      </h2>
      ...
      <div className="bg-white rounded-xl border border-gray-200 p-6">
        ...
        <div className="mb-6 flex items-center gap-6">
          <div className="text-center">
            <p className="text-3xl font-bold text-[#014D4E]">{remaining % 1 === 0 ? remaining : remaining.toFixed(1)}</p>
          ...
          <div className="flex-1 h-3 bg-gray-100 rounded-full overflow-hidden">
            <div
              ...
              style={{
                width: `${Math.min(100, Math.max(0, (remaining / ((parseFloat(total) || 1) + (parseFloat(carryOver) || 0))) * 100))}%`
              }}
            />
          </div>
        </div>
        ...
        <form onSubmit={handleSave} className="grid grid-cols-2 sm:grid-cols-4 gap-4">
          <div className="col-span-2 sm:col-span-4">
            ...
            <input
              ...
              className="w-full sm:w-48 rounded-lg border border-gray-300 px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-[#014D4E]"
            />
          </div>
          {[
