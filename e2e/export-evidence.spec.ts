...
test('export-evidence: real ZIP has the KLOE-title folder, stripped filename, and correct byte content', async ({ page }) => {
  ...
  try {
    ...
    tidy(await admin.from('kloe_evidence').delete().eq('klo_item_id', kloId), 'export-evidence: delete kloe_evidence')
...
test('export-evidence: an org with nothing uploaded gets a clear 404, not a crash', async ({ page }) => {
  ...
  try {
    ...
    tidy(await admin.from('users').delete().eq('organisation_id', orgId), 'export-evidence: delete users')
    tidy(await admin.from('organisations').delete().eq('id', orgId), 'export-evidence: delete organisations')
  }
})
