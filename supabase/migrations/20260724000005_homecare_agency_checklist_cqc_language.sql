-- ─────────────────────────────────────────────────────────────────────────────
-- Migration: 20260724000005_homecare_agency_checklist_cqc_language
-- Purpose : Replace all Homecare Agency Core checklist items with exact
--           CQC Good-level characteristic language from the Adult Social
--           Care Assessment Framework (Draft v9).
--           Insert gap items for CQC Good-level topics not previously covered.
-- Rule    : Every checklist_item is a verbatim CQC Good characteristic.
--           No paraphrasing. No homecare-specific modifications.
-- Source  : CQC Draft Assessment Framework ASC v9 (REFERENCE folder)
-- ─────────────────────────────────────────────────────────────────────────────

-- ═══════════════════════════════════════════════════════════════════════════
-- SECTION 1: UPDATE existing Homecare Agency items
-- ═══════════════════════════════════════════════════════════════════════════

-- ── SAFE ▸ Safety culture ────────────────────────────────────────────────────

UPDATE public.klo_checklist_items
SET checklist_item = $$When an incident has happened, staff are open and transparent with people and those close to them. All incidents are recorded, investigated and outcomes are communicated to those involved. If harm has occurred, people are given full details of what happened, why, and what has been learned.$$
WHERE ref = $$SAF-SC-01$$
  AND service_type_id = (SELECT id FROM public.service_types WHERE name = $$Homecare Agency$$);

UPDATE public.klo_checklist_items
SET checklist_item = $$There is a good understanding of the duty of candour.$$
WHERE ref = $$SAF-SC-02$$
  AND service_type_id = (SELECT id FROM public.service_types WHERE name = $$Homecare Agency$$);
...
UPDATE public.klo_checklist_items
SET checklist_item = $$There is a balanced and proportionate approach to risk. People's rights and choices about their care and their lives are respected. People are supported to take carefully managed risks to live fulfilling lives.$$
WHERE ref = $$SAF-MR-06$$
  AND service_type_id = (SELECT id FROM public.service_types WHERE name = $$Homecare Agency$$);
...
UPDATE public.klo_checklist_items
SET checklist_item = $$People and their representatives are involved in assessments, reviews and decisions, including the level of support needed and self-medication. This is clearly documented in care records.$$
WHERE ref = $$SAF-MT-05$$
  AND service_type_id = (SELECT id FROM public.service_types WHERE name = $$Homecare Agency$$);

UPDATE public.klo_checklist_items
SET checklist_item = $$The service actively considers opportunities to reduce the over-medication of people, in line with STOMP/STAMP principles where applicable.$$
WHERE ref = $$SAF-MT-06$$
  AND service_type_id = (SELECT id FROM public.service_types WHERE name = $$Homecare Agency$$);
...
UPDATE public.klo_checklist_items
SET checklist_item = $$The service has systems for investigating any allegations against people in positions of trust that it employs, and actions are taken to address any risk to people.$$
WHERE ref = $$SAF-SS-05$$
  AND service_type_id = (SELECT id FROM public.service_types WHERE name = $$Homecare Agency$$);
...
UPDATE public.klo_checklist_items
SET checklist_item = $$People are supported to understand what safeguarding means and how to raise concerns on behalf of themselves and others, and are encouraged and empowered to do so.$$
WHERE ref = $$SAF-SG-04$$
  AND service_type_id = (SELECT id FROM public.service_types WHERE name = $$Homecare Agency$$);

UPDATE public.klo_checklist_items
SET checklist_item = $$People are empowered to develop and maintain safe intimate relationships, where they choose to, in accordance with their rights.$$
WHERE ref = $$SAF-SG-05$$
  AND service_type_id = (SELECT id FROM public.service_types WHERE name = $$Homecare Agency$$);
...
UPDATE public.klo_checklist_items
SET checklist_item = $$Assessments focus on people's strengths, and there is a trauma-informed approach to understanding people's needs.$$
WHERE ref = $$EFF-AN-05$$
  AND service_type_id = (SELECT id FROM public.service_types WHERE name = $$Homecare Agency$$);
...
UPDATE public.klo_checklist_items
SET checklist_item = $$People receive the care, support and treatment they are entitled to from the service, which has been arranged and/or funded for them.$$
WHERE ref = $$EFF-CP-05$$
  AND service_type_id = (SELECT id FROM public.service_types WHERE name = $$Homecare Agency$$);
...
UPDATE public.klo_checklist_items
SET checklist_item = $$The service progresses any relevant advice or recommendations made by other professionals or commissioners to prevent deterioration in health and wellbeing and avoidable hospital admissions.$$
WHERE ref = $$EFF-HL-04$$
  AND service_type_id = (SELECT id FROM public.service_types WHERE name = $$Homecare Agency$$);
...
UPDATE public.klo_checklist_items
SET checklist_item = $$Staff genuinely care about people's wellbeing and show it in a thoughtful, meaningful way. They promptly respond to people's emotions, discomfort, distress, or urgent needs in a positive way. This protects their rights and dignity and staff learn to understand the causes of people's distress to avoid it happening again in the future.$$
WHERE ref = $$CAR-KD-03$$
  AND service_type_id = (SELECT id FROM public.service_types WHERE name = $$Homecare Agency$$);

UPDATE public.klo_checklist_items
SET checklist_item = $$Staff prioritise and anticipate people's comfort and wellbeing needs. They use appropriate tools and communication to meet needs effectively and to avoid any preventable discomfort, concern or distress.$$
WHERE ref = $$CAR-KD-04$$
  AND service_type_id = (SELECT id FROM public.service_types WHERE name = $$Homecare Agency$$);
...
UPDATE public.klo_checklist_items
SET checklist_item = $$People and those close to them (including representatives, advocates and where appropriate dependents) are regularly involved in planning, making and updating shared decisions about their care, support and treatment, so it is centred around them and their needs.$$
WHERE ref = $$RES-PC-03$$
  AND service_type_id = (SELECT id FROM public.service_types WHERE name = $$Homecare Agency$$);
...
UPDATE public.klo_checklist_items
SET checklist_item = $$The service seeks people's feedback and other evidence and uses this to improve access for people who are more likely to experience barriers or delays in accessing their care.$$
WHERE ref = $$RES-TA-04$$
  AND service_type_id = (SELECT id FROM public.service_types WHERE name = $$Homecare Agency$$);
...
UPDATE public.klo_checklist_items
SET checklist_item = $$When something goes wrong, people receive a sincere and timely apology and are told about any actions being taken to prevent the same thing happening again.$$
WHERE ref = $$WEL-CL-04$$
  AND service_type_id = (SELECT id FROM public.service_types WHERE name = $$Homecare Agency$$);
...
UPDATE public.klo_checklist_items
SET checklist_item = $$Staff anticipate and proactively seek to identify what is causing people's distress, and always take a least restrictive approach.$$
WHERE ref = $$DEM-SAF-01$$
  AND service_type_id = (SELECT id FROM public.service_types WHERE name = $$Homecare Agency$$);

UPDATE public.klo_checklist_items
SET checklist_item = $$People receiving their medicines covertly are supported in line with the Mental Capacity Act 2005. Staff fully understand any risk associated with mixing medicines with different foods.$$
WHERE ref = $$DEM-SAF-02$$
  AND service_type_id = (SELECT id FROM public.service_types WHERE name = $$Homecare Agency$$);
...
WHERE ref = $$DEM-SAF-03$$
  AND service_type_id = (SELECT id FROM public.service_types WHERE name = $$Homecare Agency$$);

UPDATE public.klo_checklist_items
SET checklist_item = $$Where appropriate, others close to the person, advocates, professionals and other stakeholders are meaningfully involved in the assessment of people's needs and reviews.$$
WHERE ref = $$DEM-EFF-01$$
  AND service_type_id = (SELECT id FROM public.service_types WHERE name = $$Homecare Agency$$);
...
WHERE ref = $$DEM-EFF-02$$
  AND service_type_id = (SELECT id FROM public.service_types WHERE name = $$Homecare Agency$$);

UPDATE public.klo_checklist_items
SET checklist_item = $$Staff actively consider whether people have the mental capacity to give consent. If there is a reason to doubt the person has capacity, assessments are completed in line with legal requirements and recorded.$$
WHERE ref = $$DEM-EFF-03$$
  AND service_type_id = (SELECT id FROM public.service_types WHERE name = $$Homecare Agency$$);

UPDATE public.klo_checklist_items
SET checklist_item = $$Staff can identify inappropriate and unlawful restrictive practices and use de-escalation techniques where necessary.$$
WHERE ref = $$DEM-EFF-04$$
  AND service_type_id = (SELECT id FROM public.service_types WHERE name = $$Homecare Agency$$);

UPDATE public.klo_checklist_items
SET checklist_item = $$Staff communicate clearly, in a kind and respectful way. They actively listen to people to understand and respond to their individual needs and preferences. Staff understand what privacy and dignity specifically means for the individual people they support.$$
WHERE ref = $$DEM-CAR-01$$
  AND service_type_id = (SELECT id FROM public.service_types WHERE name = $$Homecare Agency$$);

UPDATE public.klo_checklist_items
SET checklist_item = $$Staff respect people as autonomous individuals, protecting and promoting their human rights. People are actively supported to enhance their autonomy, have as much control over their lives as possible and develop new or existing skills.$$
WHERE ref = $$DEM-CAR-02$$
  AND service_type_id = (SELECT id FROM public.service_types WHERE name = $$Homecare Agency$$);

UPDATE public.klo_checklist_items
SET checklist_item = $$People receive the most appropriate and personalised care, support and treatment as the service makes reasonable adjustments where necessary.$$
WHERE ref = $$DEM-RES-01$$
  AND service_type_id = (SELECT id FROM public.service_types WHERE name = $$Homecare Agency$$);

UPDATE public.klo_checklist_items
SET checklist_item = $$Leaders use their own and people's lived experience to make informed decisions. They ask colleagues and people using the service what they think good care should look like, so that everyone feels included and valued.$$
WHERE ref = $$DEM-WEL-01$$
  AND service_type_id = (SELECT id FROM public.service_types WHERE name = $$Homecare Agency$$);

UPDATE public.klo_checklist_items
SET checklist_item = $$Staff and leaders are open and transparent, and they work collaboratively with all relevant external stakeholders and agencies.$$
WHERE ref = $$DEM-WEL-02$$
  AND service_type_id = (SELECT id FROM public.service_types WHERE name = $$Homecare Agency$$);

-- ═══════════════════════════════════════════════════════════════════════════
-- SECTION 2: INSERT new gap items
-- ═══════════════════════════════════════════════════════════════════════════

-- ── GAP 1: Safety culture — safety as a shared priority (SAF-SC-04) ──────────
...
-- ── GAP 2: Safety culture — themes and trends (SAF-SC-05) ────────────────────
INSERT INTO public.klo_checklist_items
  (klo_item_id, service_type_id, item_type, ref, checklist_item, evidence_notes, display_order, sub_service)
VALUES (
  (SELECT id FROM public.klo_items WHERE title = $$Safety culture$$),
...
-- ── GAP 3: Safety culture — closed cultures (SAF-SC-06) ──────────────────────
INSERT INTO public.klo_checklist_items
  (klo_item_id, service_type_id, item_type, ref, checklist_item, evidence_notes, display_order, sub_service)
VALUES (
  (SELECT id FROM public.klo_items WHERE title = $$Safety culture$$),
...
-- ── GAP 4: Safe environments — equipment and technology (SAF-EI-05) ──────────
INSERT INTO public.klo_checklist_items
  (klo_item_id, service_type_id, item_type, ref, checklist_item, evidence_notes, display_order, sub_service)
VALUES (
...
-- ── GAP 5: Safe systems — plans and transitions (SAF-ST-03) ──────────────────
INSERT INTO public.klo_checklist_items
  (klo_item_id, service_type_id, item_type, ref, checklist_item, evidence_notes, display_order, sub_service)
VALUES (
  ...
  $$Core$$,
  $$SAF-ST-03$$,
  $$Plans and information for care during transitions are established and shared before people move between services. Plans consider people's individual needs, circumstances, ongoing care arrangements and expected outcomes. People experience a smooth process and there is continuity of care.$$,
  NULL,
  3,
  NULL
);

-- ── GAP 6: Safe systems — needs outside remit (SAF-ST-04) ────────────────────
INSERT INTO public.klo_checklist_items
  (klo_item_id, service_type_id, item_type, ref, checklist_item, evidence_notes, display_order, sub_service)
VALUES (
  (SELECT id FROM public.klo_items WHERE title = $$Safe systems, pathways and transitions$$),
...
-- ── GAP 7: Safe systems — delegated healthcare activities (SAF-ST-05) ────────
INSERT INTO public.klo_checklist_items
  (klo_item_id, service_type_id, item_type, ref, checklist_item, evidence_notes, display_order, sub_service)
VALUES (
  (SELECT id FROM public.klo_items WHERE title = $$Safe systems, pathways and transitions$$),
...
-- ── GAP 8: Consent — best interests decisions (EFF-CT-04) ────────────────────
INSERT INTO public.klo_checklist_items
  (klo_item_id, service_type_id, item_type, ref, checklist_item, evidence_notes, display_order, sub_service)
VALUES (
...
-- ── GAP 9: Evidence-based — nutritional and hydration needs (EFF-EQ-03) ──────
INSERT INTO public.klo_checklist_items
  (klo_item_id, service_type_id, item_type, ref, checklist_item, evidence_notes, display_order, sub_service)
VALUES (
...
-- ── GAP 10: Independence — choice and control (CAR-IC-04) ────────────────────
INSERT INTO public.klo_checklist_items
  (klo_item_id, service_type_id, item_type, ref, checklist_item, evidence_notes, display_order, sub_service)
VALUES (
...
-- ── GAP 11: Listening — people confident concerns taken seriously (CAR-LR-03) ─
INSERT INTO public.klo_checklist_items
  (klo_item_id, service_type_id, item_type, ref, checklist_item, evidence_notes, display_order, sub_service)
VALUES (
...
-- ── GAP 12: Governance — clear roles and accountability (WEL-GM-06) ──────────
INSERT INTO public.klo_checklist_items
  (klo_item_id, service_type_id, item_type, ref, checklist_item, evidence_notes, display_order, sub_service)
VALUES (
...
-- ── GAP 13: Workforce equity — staff wellbeing (WEL-WE-03) ──────────────────
INSERT INTO public.klo_checklist_items
  (klo_item_id, service_type_id, item_type, ref, checklist_item, evidence_notes, display_order, sub_service)
VALUES (
...
-- ── GAP 14: Improvement — external engagement and research (WEL-IL-03) ───────
INSERT INTO public.klo_checklist_items
  (klo_item_id, service_type_id, item_type, ref, checklist_item, evidence_notes, display_order, sub_service)
VALUES (
...
-- ── GAP 15: Strategic direction — stability and sustainability (WEL-SD-03) ───
INSERT INTO public.klo_checklist_items
  (klo_item_id, service_type_id, item_type, ref, checklist_item, evidence_notes, display_order, sub_service)
VALUES (
