-- ─────────────────────────────────────────────────────────────────────────────
-- Migration: 20260724000008_extra_care_housing_checklist_cqc_language
-- Purpose : Replace all AI-written Extra Care Housing checklist items with
...
--           Delete ECH items that exceed the CQC Good-level count per KLOE.
...
-- ════════════════════════════════════════════════════════════════════════════
-- SECTION 1 — DELETE dementia sub-service items (no CQC equivalent)
-- ════════════════════════════════════════════════════════════════════════════

DELETE FROM public.klo_checklist_items
WHERE ref LIKE $$DEM-%$$
  AND service_type_id = (SELECT id FROM public.service_types WHERE name = $$Extra Care Housing$$);


-- ════════════════════════════════════════════════════════════════════════════
-- SECTION 2 — DELETE core items that exceed the CQC Good-level count per KLOE
-- ════════════════════════════════════════════════════════════════════════════

-- SAF-MR-06: CQC Managing risks has only 5 Good-level characteristics
DELETE FROM public.klo_checklist_items
WHERE ref = $$SAF-MR-06$$
  AND service_type_id = (SELECT id FROM public.service_types WHERE name = $$Extra Care Housing$$);

-- SAF-SS-05: CQC Safe staffing has only 4 Good-level characteristics
DELETE FROM public.klo_checklist_items
WHERE ref = $$SAF-SS-05$$
  AND service_type_id = (SELECT id FROM public.service_types WHERE name = $$Extra Care Housing$$);

-- EFF-AN-05: CQC Assessing needs has only 4 Good-level characteristics
DELETE FROM public.klo_checklist_items
WHERE ref = $$EFF-AN-05$$
  AND service_type_id = (SELECT id FROM public.service_types WHERE name = $$Extra Care Housing$$);

-- EFF-CP-05: CQC Care provision, integration and continuity has only 4 Good-level characteristics
DELETE FROM public.klo_checklist_items
WHERE ref = $$EFF-CP-05$$
  AND service_type_id = (SELECT id FROM public.service_types WHERE name = $$Extra Care Housing$$);

-- EFF-HL-04: CQC Supporting people to live healthier lives has only 3 Good-level characteristics
DELETE FROM public.klo_checklist_items
WHERE ref = $$EFF-HL-04$$
  AND service_type_id = (SELECT id FROM public.service_types WHERE name = $$Extra Care Housing$$);

-- CAR-KD-04: CQC Kindness, compassion and dignity has only 3 Good-level characteristics
DELETE FROM public.klo_checklist_items
WHERE ref = $$CAR-KD-04$$
  AND service_type_id = (SELECT id FROM public.service_types WHERE name = $$Extra Care Housing$$);

-- RES-PC-03: CQC Person-centred care has only 2 Good-level characteristics
DELETE FROM public.klo_checklist_items
WHERE ref = $$RES-PC-03$$
  AND service_type_id = (SELECT id FROM public.service_types WHERE name = $$Extra Care Housing$$);

-- RES-TA-04: CQC Timely and equitable access has only 3 Good-level characteristics
DELETE FROM public.klo_checklist_items
WHERE ref = $$RES-TA-04$$
  AND service_type_id = (SELECT id FROM public.service_types WHERE name = $$Extra Care Housing$$);
...
-- ════════════════════════════════════════════════════════════════════════════
-- SECTION 3 — UPDATE existing core items with verbatim CQC Good-level text
-- ════════════════════════════════════════════════════════════════════════════
...
UPDATE public.klo_checklist_items
SET checklist_item = $$People are deprived of their liberty lawfully. Any potential deprivation of liberty is recognised promptly and appropriate authorisation is sought. Where applicable, the Deprivation of Liberty Safeguards (DoLS) are used appropriately and only when it is in the best interests of the person.$$
WHERE ref = $$SAF-SG-04$$
  AND service_type_id = (SELECT id FROM public.service_types WHERE name = $$Extra Care Housing$$);
...
-- ════════════════════════════════════════════════════════════════════════════
-- SECTION 4 — INSERT gap items for all CQC Good-level characteristics
--             not covered by the remaining core items
-- ════════════════════════════════════════════════════════════════════════════

-- ── SAFE — Safety culture (gaps: SAF-SC-04, 05, 06) ─────────────────────────
...
-- ── SAFE — Safe environments and infection prevention and control (gap: SAF-EI-05)
...
-- ── SAFE — Safe systems, pathways and transitions (gaps: SAF-ST-03, 04, 05) ──
...
-- ── EFFECTIVE — Evidence-based care and equitable outcomes (gaps: EFF-EB-01, 02, 05)
...
-- ── EFFECTIVE — Consent to care and treatment (gap: EFF-CT-04) ───────────────
...
-- ── CARING — Independence, choice and control (gaps: CAR-IC-04, 05, 06) ──────
...
-- ── CARING — Equity in experiences (gap: CAR-EE-03) ──────────────────────────
...
-- ── CARING — Listening to and responding to feedback (gaps: CAR-LR-03, 04) ───
...
-- ── WELL-LED — Governance and management (gap: WEL-GM-06) ────────────────────
...
-- ── WELL-LED — Improvement, innovation and learning (gap: WEL-IL-03) ─────────
...
-- ── WELL-LED — Workforce equity and culture (gap: WEL-WE-03) ─────────────────
...
-- ── WELL-LED — Strategic direction (gap: WEL-SD-03) ──────────────────────────
